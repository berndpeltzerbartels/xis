...
class RestControllerServiceIntegrationTest {
    ...
    class FilterChainIntegrationTests {
        ...
        @Test
        @DisplayName("should return error response for unhandled controller exceptions")
        void shouldReturnErrorResponseForUnhandledControllerExceptions() {
            when(request.getHttpMethod()).thenReturn(HttpMethod.GET);
            when(request.getPath()).thenReturn("/api/users");

            doThrow(new RuntimeException("Controller error")).when(testController).getAllUsers();

            // The exception is transported to the frontend as a 500 response.
            assertDoesNotThrow(() -> restControllerService.doInvocation(request, response));

            // Verify the ResponseEntity passed to the writer contains the correct status and message.
            ...
            assertEquals("Controller error", ((ErrorResponse) responseEntity.getBody()).getMessage());
        }
