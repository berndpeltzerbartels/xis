...
- `SseEmitter.send(...)` is asynchronous and reports transport completion through its returned `CompletionStage`.
  A failed send should complete exceptionally, normally with `SseSendFailedException`, rather than only logging.
...
- `src/main/java/one/xis/http/SseSendFailedException.java`
