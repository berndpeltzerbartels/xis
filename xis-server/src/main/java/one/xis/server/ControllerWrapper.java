...
public class ControllerWrapper {
    ...
    /**
     * ID of the component. In case of page, it's an url. For frontlets, it's
    ...
    void invokeFormDataMethods(ClientRequest request, ControllerResult controllerResult) {
        SecurityUtil.checkRoles(controller.getClass(), UserContextImpl.getInstance());
        var formBinding = request.getFormBinding();
        var formMethods = formBinding == null || formBinding.isBlank()
                ? formDataMethods
                : formDataMethods.stream()
                .filter(method -> method.getFormDataKey().map(formBinding::equals).orElse(false))
                .collect(Collectors.toSet());
        var methods = MethodSorter.sortMethods(formMethods, sharedValueMethods);
        methods.stream()
    ...
    void invokeRouteMethod(ClientRequest request, ControllerResult controllerResult, ControllerMethod method) {
        SecurityUtil.checkRoles(controller.getClass(), UserContextImpl.getInstance());
        var methods = MethodSorter.sortMethods(Set.of(method), sharedValueMethods);
        methods.forEach(m -> {
            if (m.equals(method)) {
                invokeNavigationMethod(request, controllerResult, m, "route");
            } else {
    ...
    boolean isFrontletController() {
        return controller.getClass().isAnnotationPresent(Frontlet.class);
    }

    boolean isModalController() {
        return controller.getClass().isAnnotationPresent(Modal.class);
    }
    ...
    private void invokeActionMethod(ClientRequest request, ControllerResult controllerResult, ControllerMethod method) {
        ...
        invokeNavigationMethod(request, controllerResult, method, "action");
    }
        ...
        try {
            ...
            if (controllerMethodResult.getNextURL() != null) {
                ...
                controllerResult.setActionProcessing(ActionProcessing.FRONTLET);
                    ...
                    || controllerMethodResult.isReloadModalParent()) {
