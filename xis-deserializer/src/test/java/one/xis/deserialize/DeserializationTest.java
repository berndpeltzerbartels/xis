...
class DeserializationTest {
    ...
    @Test
    void deserializeMapParameter() throws NoSuchMethodException {
        var parameter = getClass().getDeclaredMethod("testMethodMap", Map.class).getParameters()[0];
        var json = "{\"a1\":\"R\",\"a8\":\"r\"}";
        var result = (Map<String, Character>) mainDeserializer.deserialize(json, parameter, userContext, new PostProcessingResults());
        assertThat(result).containsEntry("a1", 'R').containsEntry("a8", 'r');
    }

    @Test
    void deserializeMapField() throws NoSuchMethodException {
        var parameter = getClass().getDeclaredMethod("testMethodBeanParameter", TestBean.class).getParameters()[0];
        var json = "{\"charMapField\":{\"a1\":\"R\",\"a8\":\"r\"}}";
        var testBean = (TestBean) mainDeserializer.deserialize(json, parameter, userContext, new PostProcessingResults());
        assertThat(testBean.getCharMapField()).containsEntry("a1", 'R').containsEntry("a8", 'r');
    }

    @Test
    void deserializeNestedListOfMapsField() throws NoSuchMethodException {
        var parameter = getClass().getDeclaredMethod("testMethodBeanParameter", TestBean.class).getParameters()[0];
        var json = "{\"charMapListField\":[{\"a1\":\"R\",\"a8\":\"r\"},{\"e4\":\"P\"}]}";
        var testBean = (TestBean) mainDeserializer.deserialize(json, parameter, userContext, new PostProcessingResults());
        assertThat(testBean.getCharMapListField()).hasSize(2);
        assertThat(testBean.getCharMapListField().get(0)).containsEntry("a1", 'R').containsEntry("a8", 'r');
        assertThat(testBean.getCharMapListField().get(1)).containsEntry("e4", 'P');
    }
    ...
    @SuppressWarnings("unused")
    void testMethodMap(@FormData("map") Map<String, Character> map) {
    ...
    static class TestBean {
        ...
        private Map<String, Character> charMapField;

        private List<Map<String, Character>> charMapListField;
    }
