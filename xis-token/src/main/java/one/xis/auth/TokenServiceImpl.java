...
class TokenServiceImpl implements TokenService {
    ...
    public ApiTokens newTokens(UserInfo userInfo, String issuer, String keyId, KeyPair keyPair) {
        ...
        accessTokenClaims.setRealmAccess(new AccessTokenClaims.RealmAccess(roles));
    ...
    private <C extends TokenClaims> void validateClaims(C claims) throws InvalidTokenException {
        ...
        if (claims.getClientIdOrAuthorizedParty() == null || claims.getClientIdOrAuthorizedParty().isBlank()) {
            throw new InvalidTokenException("Token client_id or azp is missing or empty");
        }
        ...
        if (claims.getNotBeforeSeconds() != null) {
            ...
            if (notBefore.minus(CLOCK_SKEW).isAfter(now)) {
                throw new InvalidTokenException("Token not valid yet (nbf in the future)");
            }
        }
        if (Instant.ofEpochSecond(claims.getIssuedAtSeconds()).minus(CLOCK_SKEW).isAfter(now)) {
