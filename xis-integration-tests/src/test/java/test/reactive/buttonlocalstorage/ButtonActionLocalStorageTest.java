...
class ButtonActionLocalStorageTest {
    ...
    void standaloneButtonLocalStorageActionsWork() {
        // Open the page
        var client = testContext.openPage(ButtonActionLocalStoragePage.class);

        // Debug: Was steht wirklich im HTML?
        var counterElement = client.getDocument().getElementById("counter-value");

        // Click increment button
        var incrementButton = client.getDocument().getElementById("increment-standalone");
        incrementButton.click();

        // Debug: Was passiert nach dem Klick?
        counterElement = client.getDocument().getElementById("counter-value");

        // Check that the action worked using the new parsed methods
        assertThat(counterElement.getInnerText()).isNotNull();
        assertThat(client.getLocalStorage().getItem("counter")).isNotNull();

        // Test the new convenient API
        Integer counterValue = client.getLocalStorage().getIntValue("counter");
    ...
    void localStorageButtonsHaveCorrectType() {
        ...
        var incrementButton = client.getDocument().getElementById("increment-standalone");
        var decrementButton = client.getDocument().getElementById("decrement-standalone");
    ...
    void localStorageReactiveUpdatesWork() {
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            Integer localStorageValue = client.getLocalStorage().getIntValue("counter");
