...
class ActionLinkFrontletTest {
    ...
    private ActionLinkFrontletService service;
    private IntegrationTestContext testContext;

    @BeforeEach
    void init() {
        service = mock(ActionLinkFrontletService.class);
        when(service.getData()).thenReturn(new ActionLinkFrontletData(101, "bla"));

        testContext = IntegrationTestContext.builder()
                .withSingleton(ActionLinkFrontlet.class)
                .withSingleton(SimpleFrontlet.class)
                .withSingleton(FrontletPage.class)
                .withSingleton(service)
                .withSingleton(IndexPage.class) // new page in action3
                .build();

        testContext.getSingleton(FrontletPage.class).setFrontletId("ActionLinkFrontlet");
    }

    @Test
    void action1() {
        var client = testContext.openPage(FrontletPage.class);
    ...
    @Test
    @DisplayName("Action is a variable and link of a frontlet is clicked, but the result demands displaying a new page")
    void action3() {
        var client = testContext.openPage(FrontletPage.class);
    ...
    @Test
    @DisplayName("Action demands displaying another frontlet")
    void action4() {
        var client = testContext.openPage(FrontletPage.class);
