...
class RefreshEventTest {
    ...
    void init() {
        context = IntegrationTestContext.builder()
                ...
                .withSingleton(MatchSummaryPage.class)
    ...
    @Test
    @DisplayName("simulatePushEvent() refreshes frontlet that listens to the event key")
    void simulatePushEventRefreshesFrontlet() {
        var client = context.openPage(MatchPage.class);
    ...
    @Test
    @DisplayName("Refresh event with wrong key does not refresh the frontlet")
    void wrongEventKeyDoesNotRefreshFrontlet() {
        var client = context.openPage(MatchPage.class);
        ...
        // Score should still show 0:0 - frontlet was not refreshed
        assertThat(client.getDocument().getElementById("home-goals").getInnerText())
                .isEqualTo("0");
    ...
    @Test
    @DisplayName("Refresh event does not refresh a frontlet from a previously displayed page")
    void refreshEventDoesNotRefreshDetachedFrontlet() {
        var client = context.openPage(MatchPage.class);

        assertThat(client.getDocument().getElementById("home-goals").getInnerText())
                .isEqualTo("0");

        client.getDocument().getElementById("summary-link").click();
        context.getSingleton(ScoreBoard.class).setScore(2, 0);
        context.simulatePushEvent("score-updated");

        assertThat(client.getDocument().getElementById("summary").getInnerText())
                .isEqualTo("Match summary");
        assertThat(client.getDocument().getElementById("home-goals"))
                .isNull();
    }
