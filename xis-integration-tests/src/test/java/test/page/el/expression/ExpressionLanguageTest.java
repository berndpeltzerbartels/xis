...
class ExpressionLanguageTest {
    ...
    @Test
    void evaluatesDeferredSourceAttribute() {
        var client = testContext.openPage("/expressionLanguage.html");

        var image = client.getDocument().getElementById("deferred-src");
        assertThat(image.getAttribute("src")).isEqualTo("/assets/first.png");
        assertThat(image.hasAttribute("xis:src")).isFalse();
    }
