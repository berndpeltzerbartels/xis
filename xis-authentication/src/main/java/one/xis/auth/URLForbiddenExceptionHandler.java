...
class URLForbiddenExceptionHandler implements ControllerExceptionHandler<URLForbiddenException> {
    ...
    private String loginUrl(String url) {
        if (externalIDPServices.getExternalIDPServices().size() == 1 && !hasLocalLogin()) {
            // If exactly one external IDP service is available and local login is not available,
        ...
        // If multiple IDP services are available or local login is present, we redirect to the default login page.
        // This allows the user to choose which IDP to use for authentication or use a local login.
