...
class ELFunctionsTest {
    ...
    @Nested
    @DisplayName("user role functions")
    class UserRoleFunctionsTest {

        @Test
        @DisplayName("isUserInRole returns true for current role")
        void isUserInRoleReturnsTrueForCurrentRole() throws ScriptException {
            var result = evaluate("isUserInRole('ADMIN')", "{value: true}", "['USER', 'ADMIN']");

            assertThat(result.asBoolean()).isTrue();
        }

        @Test
        @DisplayName("isUserInRole returns false for missing role")
        void isUserInRoleReturnsFalseForMissingRole() throws ScriptException {
            var result = evaluate("isUserInRole('ADMIN')", "{value: true}", "['USER']");

            assertThat(result.asBoolean()).isFalse();
        }

        @Test
        @DisplayName("isUserInRoles accepts multiple singular arguments")
        void isUserInRolesAcceptsMultipleSingularArguments() throws ScriptException {
            var result = evaluate("isUserInRoles('SUPPORT', 'ADMIN')", "{value: true}", "['USER', 'ADMIN']");

            assertThat(result.asBoolean()).isTrue();
        }

        @Test
        @DisplayName("isUserInRoles accepts an array")
        void isUserInRolesAcceptsArray() throws ScriptException {
            var result = evaluate("isUserInRoles(roles)", "{roles: ['SUPPORT', 'ADMIN']}", "['USER', 'ADMIN']");

            assertThat(result.asBoolean()).isTrue();
        }

        @Test
        @DisplayName("role functions are false without a current response")
        void roleFunctionsAreFalseWithoutCurrentResponse() throws ScriptException {
            var testScript = javascript + """

                    var expressionParser = new ExpressionParser(elFunctions);
                    var expression = expressionParser.parse("isUserInRole('ADMIN')");
                    expression.evaluate(new Data({}));
                    """;

            assertThat(JSUtil.execute(testScript, BrowserFunctions.BINDINGS).asBoolean()).isFalse();
        }
    }
    ...
    private Value evaluate(String expression, String data, String roles) throws ScriptException {
        var testScript = (javascript + """
                
                    var app = {currentResponse: {userRoles: ${roles}}};
                    var data = new Data(${data});
                    var expressionParser = new ExpressionParser(elFunctions);
                    var expression = expressionParser.parse("${expression}");
                    expression.evaluate(data);
                """).replace("${expression}", expression).replace("${data}", data).replace("${roles}", roles);
