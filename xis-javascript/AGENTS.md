...
- The Gradle JavaScript plugin merges many source files into generated bundles. It analyzes `class ... extends ...`
  relationships and sorts files so base classes are written before subclasses. When moving JavaScript classes into
  separate files, keep each class declaration parseable by `JsContentAnalyzer` and check `JSFileSorter` behavior.
