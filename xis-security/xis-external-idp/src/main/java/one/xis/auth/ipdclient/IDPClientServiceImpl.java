...
class IDPClientServiceImpl implements IDPClientService {
    ...
    @EventListener
    public void localUrlAssigned(LocalUrlAssignedEvent e) {
        idpClientConfigs.stream().map(config -> idpClientFactory.createConfiguredIDPClient(config, e.localUrl()))
    ...
    public String loginFormUrl(String idpId, String redirectUri) {
        ...
        var state = StateParameter.create(redirectUri, idpId);
