...
@Action
void deleteProduct(@ActionParameter("productId") long productId) {
...
The action method can then return `void`, a page class, `PageResponse`, a frontlet class, `FrontletResponse`, or
`ModalResponse`.
...
A form inside a modal usually returns `ModalResponse.close()` or `ModalResponse.close().reloadParent()`. If validation
fails, the modal stays open and displays validation messages like any other XIS form.
...
@Action
PageResponse openProduct(@ActionParameter("productId") long productId) {
...
class CartFrontlet {
    ...
    @Action
    PageResponse checkout(@ActionParameter("cartId") long cartId) {
...
```java
@Action
String openProductUrl(@ActionParameter("productId") long productId) {
...
@Action
PageUrlResponse openReport(@ActionParameter("reportId") String reportId) {
...
class ProductDetailsFrontlet {
    ...
    @ModelData
    Product product(@FrontletParameter("productId") long productId) {
...
@Action
FrontletResponse showProduct(@ActionParameter("productId") long productId) {
...
You can also address a frontlet by its frontlet id or by a public frontlet URL. URL style is especially useful for
distributed applications, because the caller does not need the target frontlet class on its classpath.

```java
@Action
FrontletResponse showProductByUrl(@ActionParameter("productId") long productId) {
    return new FrontletResponse("/product-summary?productId=" + productId)
            .targetContainer("main");
}
```

The target frontlet declares the URL:
...
```java
@Action
FrontletResponse showPreview(@ActionParameter("productId") long productId) {
...
Use `ModalResponse` when an action should open or close a modal dialog.
...
@Action
ModalResponse editCustomer(@ActionParameter("customerId") long customerId) {
    return ModalResponse.open(EditCustomerModal.class)
            .parameter("customerId", customerId);
}
```

To save and close a modal:
...
`reloadParent()` reloads the page that opened the modal, or exactly the frontlet instance that opened it. It is not a
broadcast mechanism and it does not reload all frontlets of the same type. Use [events](events.md) when other clients or
unrelated visible UI parts need to refresh.

See [Modals](modals.md) for the full modal workflow.
...
| Decide the target view on the server before a page is selected | Optional [router](routers.md) with `@Router` and `@Route` |
...
| Open or close a modal dialog | `xis:modal` or `ModalResponse` |
