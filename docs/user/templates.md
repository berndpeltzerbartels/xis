...
For attributes that make the browser load a resource immediately, use the `xis:*` form so the browser does not request
the raw template expression before XIS has initialized the page:
...
| Function | Use |
| --- | --- |
| `isUserInRole(role)` | Returns `true` when the current authenticated user has the named role. |
| `isUserInRoles(role, ...)` | Returns `true` when the current authenticated user has at least one of the named roles. Also accepts an array. |

These functions run in the browser and are only for presentation logic, usually with `xis:if`. They can hide or show
controls, but hidden frontend markup is not access control. Protect the page, frontlet, action, parameter, or DTO with
`@Authenticated` or `@Roles` whenever the operation itself must be restricted.
...
Simple drag-and-drop actions use `xis:drag` and `xis:drop`:
...
Read [Drag and drop](drag-and-drop.md) for the full behavior and test support.
